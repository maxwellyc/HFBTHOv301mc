...
Module UNEDF
 ...
 Contains
  ...
  Subroutine read_UNEDF_NAMELIST(fname,noForces)
       ...
       Else
           ...
           Case ('UNE1')
              ...
              if(use_3N_couplings) then
                 use_3N_couplings = .False.
                 override_3N_couplings = .True.
              end if  !EOedit: changed 'endif' to 'end if'
           !===================================================================================   
           !EOedit: Modified UNEDF1 functional (Y. She et al., Phys. Rev. C. 89, 034309 (2014))
           !===================================================================================
           Case ('UNE1-SO')
              inforcedname='UNE1-SO'
              use_Namelist=.False.
              ! kind of the functional
              use_INM  = .True.
              ! Surface coefficients
              CrDr(0)  =  -45.135131022237300_pr
              CrDr(1)  = -145.382167908057000_pr
              CpV0(0)  = -186.065399575124000_pr  !From UNEDF1
              CpV0(1)  = -206.579593890106000_pr  !From UNEDF1
              !CpV0(0)  = -205.1_pr   !EOedit: From equation 3 of the paper above (scaled pairing, SHFB+LN)
              !CpV0(1)  = -252.5_pr   !EOedit: From equation 3 of the paper above (scaled pairing, SHFB+LN)
              CrdJ(0)  =  -88.050_pr     !EOedit: Spin-orbit (SO) parameter changed
              CrdJ(1)  =  8.458_pr       !EOedit: Spin-orbit (SO) parameter changed
              CJ(0)    =    0.000000000000000_pr
              CJ(1)    =    0.000000000000000_pr
              CExPar   =    1.000000000000000_pr
              ! Various
           ...
           Case ('DME_N2LOD')
              ...
              if(use_3N_couplings) then
                 ! Surface coefficients
                 CrDr(0)  =  -43.158325871977000_pr
                 CrDr(1)  = -112.858756291090000_pr
                 CpV0(0)  = -184.777940626164000_pr
                 CpV0(1)  = -203.949602634574000_pr
                 CrdJ(0)  =  -86.819259306787200_pr
                 CrdJ(1)  =  -70.971108325169700_pr
                 CJ(0)    =  -92.657483068129900_pr
                 CJ(1)    =  -28.648957783068600_pr
                 ! Associated INM parameters
                 RHO_NM   =    0.152661660863980_pr
                 E_NM     =  -15.800000000000000_pr
                 K_NM     =  260.000000000000000_pr
                 ASS_NM   =   28.613115132525000_pr
                 LASS_NM  =   43.462114575160300_pr
                 SMASS_NM =    1.085501374241110_pr
                 call set_3n_couplings(fname)
              endif
           Case ('REG_LO')
              inforcedname='REG_LO'
    ...
    Case ("UNE1-SO")
       FunctionalName='UNE1-SO'  !EOedit for UNEDF1-SO
    Case default
  ...
  Subroutine skforce(fname,noForces)
    ...
    Case ('SKM*')
        ...
        CpV1=0.5_pr
        !CpV0=(/ -265.2500_pr, -340.0625_pr /)
        CpV0=(/ -217.74955_pr, -221.81255_pr /)   !EOedit: Test to find CpV0 for the HFBTHOv133 mass tables
        !CpV0=(/ -221.81255_pr, -221.81255_pr /)  !EOedit: possibly the CpV0 for the HFBTHOv133 mass tables? (SkM*)
    !---------------------------------------------------------------------
    ...
    Case ('SKP')
        ...
        b4=wls/two; b4p=wls/two
        ! pp-Forces
        CpV1=0.50_pr   !EOedit: CpV1 for the HFBTHOv133 mass tables?
        !CpV1=0.50_pr
        !CpV0=(/ -265.2500_pr, -340.0625_pr /)
        CpV0=(/ -221.81255_pr, -221.81255_pr /)   !EOedit: CpV0 for the HFBTHOv133 mass tables?
        ! In hfbtho_v166p, this value for LN on is -200.750, -201.500 MCcomment
        ! 1/29/2019
    !---------------------------------------------------------------------
    ...
    Case ('SLY4')
        ...
        CpV0=(/ -258.200_pr, -258.200_pr /) ! EOedit: this seems to be the CpV0 for the HFBTHOv133 mass tables (SLy4)
        ! In hfbtho_v166p, this is -260.250, -260.400 MCcomment 1/29/2019
    !---------------------------------------------------------------------
    ...
    Case ('SKX')
        ...
        CpV0=(/ -259.0391_pr, -274.8433_pr /)
    !-------------------------------------------------------------------------
    ! SV-min force, P. Klupfel et. al., Phys. Rev. C. 79, 034310 (2009) EOedit    
    !-------------------------------------------------------------------------
    Case ('SV-min')
        ! ph-Force
        noForces=1
        ...
        !hbzero = 20.73_pr
        hb0_charge_dependent = .True.
        hbzeron = 20.7212600999999985_pr
        hbzerop = 20.7498207000000008_pr
        hbzero = 0.5_pr*(hbzeron + hbzerop)   !Must be the average between proton and neutron values
        t0 =-0.211224824999999964d+04; x0 = 0.243886305999999831_pr
        t1 = 0.295780847999999935d+03; x1 =-1.43492628000000111_pr
        t2 = 0.142268038000000331d+03; x2 =-2.62589911999999659_pr
        t3 = 0.139885671000000002d+05; x3 = 0.258070167000000406_pr
!        wls= 0.14990000d+03; 
        sigma=0.255367889999999986
        b4=55.6454295000000059_pr; b4p=45.9361510000000024_pr
        ! pp-Forces
        CpV1=0.50_pr
        CpV0=(/ -214.280000000000001_pr, -214.280000000000001_pr /)
    !---------------------------------------------------------------------
...
!        t0 =  -1128.75_pr; x0 = 0.45_pr;
  ...
  Subroutine set_functional_parameters(fname,lpr)
    ...
    If(FunctionalName=='SV-min') Then !EOedit for SV-min
      RHO_NM=0.211591070_pr
      E_NM=0.00_pr
      P_NM=0.00_pr
      K_NM=0.00_pr
      ASS_NM=0.00_pr
